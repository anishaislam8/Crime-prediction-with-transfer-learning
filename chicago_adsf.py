        ...
        for j in range(len(ri_index)):
            ...
            if len(intersection) == 0:
                ...
                for k in range(len(intersection)):
                    intersection_ri_alli.append(
                        ri_all[i][ri_index[i].tolist().index(intersection[k])])
                    intersection_ri_allj.append(
                        ri_all[j][ri_index[j].tolist().index(intersection[k])])
                ...
                if len(union_rest) == 0:
                    ...
                    for k in range(len(union_rest)):
                        if union_rest[k] in ri_index[i]:
                            union_ri_alli.append(
                                ri_all[i][ri_index[i].tolist().index(union_rest[k])])
                        else:
                            union_ri_allj.append(
                                ri_all[j][ri_index[j].tolist().index(union_rest[k])])
...
class GraphAttentionLayer(nn.Module):
    # previously negative slope 0,2
    def __init__(self, in_features, out_features, n_heads, is_concat=True, dropout=0.3, leaky_relu_negative_slope=0.2):
...
def preprocessing():
    ...
    for line in lines:
        ...
        desired_array = [int(numeric_string)
                         for numeric_string in current_array]
...
class ADSF(torch.nn.Module):
    ...
    def forward(self, road_representation, poi_representation, inflows, outflows, batch_no):
        ...
        structural_interaction = structural_interaction.resize_(
            (structural_interaction.shape[0], structural_interaction.shape[1], 1))
        chicago_adjacency_matrix = torch.tensor(chicago_adjacency_matrix)
        chicago_adjacency_matrix = chicago_adjacency_matrix.resize_(
            (chicago_adjacency_matrix.shape[0], chicago_adjacency_matrix.shape[1], 1))
        ...
        for i in range(77):
            result = torch.vstack(
                (road_network[i]*0.167, poi[i]*0.501, inflows[i][batch_no]*0.166, outflows[i][batch_no]*0.166))
            feature_matrix.append(result)

        biggest = 0
        for i in range(77):
            if feature_matrix[i].shape[0] * feature_matrix[i].shape[1] > biggest:
                biggest = feature_matrix[i].shape[0] * \
                    feature_matrix[i].shape[1]

        new_matrix = []
        for i in range(77):
            temp = list(torch.reshape(
                feature_matrix[i], (feature_matrix[i].shape[0] * feature_matrix[i].shape[1],)))
        ...
        if self.layer1 == None:
            self.layer1 = GraphAttentionLayer(
                self.in_features, self.out_features, self.n_heads, self.is_concat, self.dropout)

        feature_tensor = new_matrix
        feature_tensor = feature_tensor.type(torch.FloatTensor)
        feature_representation = self.layer1(
            feature_tensor, chicago_adjacency_matrix.float(), structural_interaction.float())
