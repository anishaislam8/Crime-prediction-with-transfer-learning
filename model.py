...
class ModifiedAIST(nn.Module):
    def __init__(self, region, in_hgat, in_fgat, out_gat, att_dot, nhid_rnn, nlayer_rnn, att_rnn, ts, batch_size, nclass=2):
        ...
        self.sab1 = self_LSTM_sparse_attn_predict(out_gat+new_size, nhid_rnn, nlayer_rnn, truncate_length=5, top_k=4, attn_every_k=5)
    ...
    def forward(self, region, batch, batch_size, similar_region_number, target_cat_chicago, loaded_data, train_or_test, current_crime_category, inflows, outflows):
        ...
        region_feature_representations = self.adsf(road_representation, poi_representation, inflows, outflows, batch) # passing 77 values
        ...
        for att in range(similar_region_number):
            if denominator == False:
                val = (((attention_values[similar_regions[att]] - minimum)*1.0)/((maximum - minimum)*1.0))
            else:
                val = (((attention_values[similar_regions[att]] - minimum)*1.0)/(0.00001))
        ...
        for att in range(similar_region_number):
            attention_values[similar_regions[att]] = normalized_attention_list[att]
        ...
        train_y_tl, test_y_tl = similar_region_crimes(loaded_data, similar_regions, similar_region_number, target_cat_chicago)
        if train_or_test == 0:
        ...
        final_crime_representation_for_this_batch = crime_representation(train_or_test, similar_regions, attention_values, current_crime_category, similar_region_number, batch)
        feature = region_feature_representations[region].reshape((20,new_size))
        temp = []
        for j in range(batch_size):
            temp.append(torch.reshape(feature, (1, feature.shape[0], feature.shape[1])))
        feature = torch.empty(batch_size, feature.shape[0], feature.shape[1])
        feature = torch.cat(temp)
        lstm_input = torch.concat((final_crime_representation_for_this_batch, feature), axis = 2)
