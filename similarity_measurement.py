...
class Pearson(torch.nn.Module):
    ...
    def forward(self, chicago_target_region_representation, batch_no, train_or_test):
        ...
        for i in range(77):
            ...
            correlation_values.append(sys.float_info.min if math.isnan(
                correlation[0][1]) else correlation[0][1])
...
class Similarity(torch.nn.Module):
    def __init__(self, region):
        ...
        self.dropout = 0.3
        self.layer1 = SparseGraphAttentionLayer(
            self.in_features, self.out_features, self.dropout)
    ...
    def forward(self, chicago_region_representations):
        ...
        # returns a 77x1 sized torch with similarity score for each region of chicago
        return attentions
