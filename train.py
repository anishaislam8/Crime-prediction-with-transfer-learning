...
argp.add_argument("--in_hgat", default=1, type=int,
                  help="input dimension of hgat layers")
argp.add_argument("--in_fgat", default=12, type=int,
                  help="input dimension of fgat layers:num of features")
argp.add_argument("--out_gat", default=8, type=int,
                  help="output dimension of both hgat and fgat layers")
argp.add_argument("--att_dot", default=40, type=int,
                  help="(dot-product)attention dimension of fgat")
argp.add_argument("--nhid_rnn", default=40, type=int,
                  help="hidden dimension of rnn")  # previous 40
argp.add_argument("--nlayer_rnn", default=1, type=int,
                  help="number of rnn layers")  # previous 1
argp.add_argument("--att_rnn", default=30, type=int,
                  help="(location)attention dimension of temporal module")
...
loaded_data = torch.from_numpy(np.loadtxt(
    "chicago_data/" + location + "/com_crime/r_" + str(chicago_region) + ".txt", dtype=int)).T
...
# load data
train_x, train_x_daily, train_x_weekly, train_y, test_x, test_x_daily, test_x_weekly, test_y = load_self_crime(
    x, x_daily, x_weekly, Y)
...
model = ModifiedAIST(chicago_region, in_hgat, in_fgat, out_gat,
                     att_dot, nhid_rnn, nlayer_rnn, att_rnn, ts, batch_size)
...
for epoch in range(epochs):
    ...
    for i in range(train_batch):  # 33
        ...
        model.train()
        optimizer.zero_grad()
        output, y_true = model(chicago_region, i, batch_size, k,
                               crime_map[current_crime_category], loaded_data, 0, current_crime_category, inflows_train, outflows_train)
        ...
        files = glob.glob('best_epoch_models_chicago/' +
                          str(chicago_region)+'/'+str(current_crime_category) + '/*.pkl')
        for file in files:
            remove_file = file
            file = file.split("/")
...
plt.savefig("Training Losses/" + str(chicago_region) + "_" +
            str(current_crime_category) + "_" + str(k) + "_MAE_loss_weighted_new.jpg")
...
for i in range(test_batch):
    model.eval()
    output_test, y_similar_regions_chicago = model(
        chicago_region, i, batch_size, k, crime_map[current_crime_category], loaded_data, 1, current_crime_category, inflows_test, outflows_test)
    y_test = Variable(test_y[i]).float()
    y_test = y_test.view(-1, 1)
    y_test = torch.from_numpy(scale.inverse_transform(y_test))
    print("True", y_test)
    output_test = torch.from_numpy(
        scale.inverse_transform(output_test.detach()))
